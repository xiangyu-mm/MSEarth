...
# 初始化本地 OpenAI 客户端
client = OpenAI(
    api_key="EMPTY",  # 本地部署时无需实际 API Key
...
def get_multiple_answers(figure_path, prompt, model, num_answers=8, temperature=1.0):
    """
    生成多个候选答案。
    """
    ...
    for i in range(num_answers):
        try:
            ...
            print(f"生成候选答案时发生错误：{e}")
            answers.append({"error": str(e)})
    return answers

def get_cot_answer(figure_path, prompt, model, temperature=0):
    """
    生成一个 CoT 答案。
    """
    base64_image = encode_image(figure_path)
...
def remove_trailing_commas(json_str):
    """
    移除 JSON 字符串中对象或数组末尾多余的逗号。
    """
    json_str = re.sub(r',\s*([}\]])', r'\1', json_str)
    return json_str

def process_with_multiple_answers(data, figure_path, prompt, model, num_answers=8, temperature=1.0):
    """
...
def process_with_cot_answer(data, figure_path, prompt, model, temperature=0):
    """
...
class DatasetProcessor:
    ...
    def get_prompt(self, query=None, cot=False):
        # query = re.sub(r'Caption:.*?Question:', 'Question:', query, flags=re.DOTALL)
        if cot:
            prompt = f"""
            ...
            Based on the image, provide a step-by-step reasoning process to derive the correct answer. Then, state the final answer.

            The output must be written in **JSON format** using the structure below:
            ```json
            {{
                "reasoning_chain": [step1, step2,..], //Please solve the problem step by step
    ...
    def process_single_data(self, data):
        ...
        data = process_with_multiple_answers(
            ...
            prompt=prompt,
            model=self.model,
            num_answers=8,  # 生成8个候选答案
            temperature=0.7  # 设置温度为1.0
        )
...
if __name__ == '__main__':
    ...
    processor = DatasetProcessor(
        ...
        save_path=os.path.join(args.save_dir, f"mcq_results_{args.model}.json"),
        model=args.model
    )
