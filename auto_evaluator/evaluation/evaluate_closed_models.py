...
class DatasetProcessor:
    def __init__(self, file_path, save_path, task_type, model, max_workers=4):
    ...
    def process_single_data(self, data):
        ...
        query = data.get("query", "").replace("<image>", "")
    ...
    def process_file(self):
        ...
        with ThreadPoolExecutor(max_workers=self.max_workers) as executor:
            futures = [executor.submit(self.process_single_data, data) for data in self.data_list]
            for future in tqdm(as_completed(futures), total=len(self.data_list), desc=f"Processing {self.task_type} task with {self.model}"):
...
if __name__ == '__main__':
    ...
    parser.add_argument("--max-workers", type=int, default=10, help="Number of threads for concurrent processing")
